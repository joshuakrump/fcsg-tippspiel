...
export async function importMatchesForMonth(formData: FormData) {
  ...
  if (!response.ok) {
    throw new Error(`API-Football Fehler: HTTP ${response.status}`);
  }
  ...
  for (const item of fixtures) {
    ...
    const opponentName = localTeam?.name ?? opponentApiTeam.name;
    const opponentLogo = localTeam?.logo_path ?? null;
    ...
    if (upsertError) {
      throw new Error(
        `Spiel ${fixtureId} konnte nicht importiert/aktualisiert werden: ${upsertError.message}`
      );
    }
  }
...
export async function syncMatchWithApi(formData: FormData) {
  ...
  if (!match.api_fixture_id) {
    throw new Error("Dieses Spiel besitzt keine API-Fixture-ID.");
  }
